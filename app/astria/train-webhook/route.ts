...
type ReplicateTrainingOutput = {
  version?: string;
  weights?: string; // Aquí es donde se espera la URL final
};
...
type ReplicateTraining = {
  ...
  input: any;
  error: string | null;
  output?: ReplicateTrainingOutput; // Se agrega la propiedad output
};
...
export async function POST(request: Request) {
  ...
  if (training.status === "succeeded") {
    ...
    // Ahora tomamos la URL desde training.output.weights
    if (!training.output || !training.output.weights) {
      console.error('❌ No se encontró "weights" en el objeto output del entrenamiento');
      return NextResponse.json(
        { message: "No se encontró la URL en weights." },
        { status: 500 }
      );
    }

    const hfLoraUrl = training.output.weights;
    
    console.log('📝 Actualizando modelo en DB:', { model_id, hfLoraUrl });
    const { data: modelUpdated, error: modelUpdateError } = await supabase
      .from("models")
      .update({
        modelId: hfLoraUrl,
        status: "finished",
